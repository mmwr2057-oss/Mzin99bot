...
# Headers لتجنب الحظر على السيرفرات السحابية
HEADERS = {
...
async def fetch_market_sentiment_async(session, symbol):
    ...
    try:
        ...
        async with session.get(global_url, headers=HEADERS, timeout=5) as g_resp, session.get(top_url, headers=HEADERS, timeout=5) as t_resp:
...
async def run_scan(query, interval, mode):
    ...
    if not results:
        await query.message.edit_text("❌ حدث خطأ أو لم يتم العثور على بيانات.", reply_markup=InlineKeyboardMarkup(keyboard_back))
        return
...
async def run_scan_ls_posit(query, interval):
    ...
    if not results:
        await query.message.edit_text("❌ لا توجد عملات بنسب تغير موجبة حالياً (أو قيد الاتصال ببيانات الحيتان).", reply_markup=InlineKeyboardMarkup(keyboard_back))
        return
...
async def run_scan_ls_neg(query, interval):
    ...
    if not results:
        await query.message.edit_text("❌ لا توجد عملات بنسب تغير سالبة حالياً (أو قيد الاتصال ببيانات الحيتان).", reply_markup=InlineKeyboardMarkup(keyboard_back))
        return
